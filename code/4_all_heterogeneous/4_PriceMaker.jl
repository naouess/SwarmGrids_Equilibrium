...
begin
    ...
    set_optimizer_attribute(model, "convergence tolerance", 1e-6)
    ...
    # Set start values
    set_start_value.(S, [0.193, 0.153, 5.432])
    set_start_value.(B, [0.000, 2.298, 39.320] * factor)
    ...
    @variable(model, μ_m[i in nanogrids] >= 0)
    ...
    startvals = fill(0.1, length(timesteps), length(nanogrids))
    set_start_value.(λ_nano, startvals)

    startvals_1 = fill(0.2, length(timesteps))
    set_start_value.(λ_micro, startvals_1)

    @expression(model, p_nano[t in timesteps, i in nanogrids],
                     α_nano - β_nano * (E_sol[t, i] + E_batminus[t, i] - E_batplus[t, i] + E_imp[t, i] - E_exp[t, i]))

    @expression(model, p_micro[t in timesteps],
                       λ_micro[t])

    # Complementarity constraints
    @constraint(model, poeq1[t in timesteps, i in nanogrids], - λ_nano[t, i] + μ_sol[t, i] + β_nano * e_dem_df[t, i] ⟂ E_sol[t, i])
    @constraint(model, poeq2[t in timesteps, i in nanogrids], c_bat + λ_nano[t, i] + μ_batplus[t, i] - u_bat* λ_SoC[t, i] - β_nano * e_dem_df[t, i] ⟂ E_batplus[t, i]) 
    @constraint(model, poeq3[t in timesteps, i in nanogrids], c_bat - λ_nano[t, i] + μ_batminus[t, i] +  1/u_bat* λ_SoC[t, i] + β_nano * e_dem_df[t, i] ⟂ E_batminus[t, i])
    @constraint(model, poeq4[t in timesteps, i in nanogrids], c_ie - λ_nano[t, i] + p_micro[t] + λ_micro[t]  + μ_imp[t, i] + β_nano * e_dem_df[t, i] ⟂ E_imp[t, i])
    @constraint(model, poeq5[t in timesteps, i in nanogrids], c_ie + λ_nano[t, i] - p_micro[t] - λ_micro[t] + μ_exp[t, i] - β_nano * e_dem_df[t, i] ⟂ E_exp[t, i]) 
    @constraint(model, poeq6[i in nanogrids], c_S[i] * (1 + μ_m[i]) - sum(t -> e_solar[t] * μ_sol[t, i], timesteps) ⟂ S[i])
    @constraint(model, poeq7[i in nanogrids], c_B[i] * (1 + μ_m[i]) - z_B * λ_SoC[1, i] - v_B * sum(t -> μ_batplus[t, i] + μ_batminus[t, i], timesteps) - sum(t -> μ_SoC[t, i], timesteps) ⟂ B[i])
    ...
    @constraint(model, IneqCon7[i in nanogrids], m[i] - S[i] * c_S[i] - B[i] * c_B[i] ⟂ μ_m[i])
    ...
    @expression(model, indiprofit[i in nanogrids], 
        ...
        sum(t -> p_nano[t, i] * (E_sol[t, i] + E_batminus[t, i] + E_imp[t, i] - E_batplus[t, i] - E_exp[t, i]), timesteps)
...
println("Individual Costs: ", indicost)
...
xticks_days = 0:hours_per_day*4:ndays*hours_per_day*4
xtick_labels = ["$(i)" for i in 0:ndays]

# --- Single plot for all nanogrids ---
p_all_nano = plot(
    ylabel="\nPrice / dual variable in €/kWh",
    xlabel="\nTime (days)\n\n",
    ylim=[0, 1.1],
    legend=:left,
    xticks=(xticks_days, xtick_labels),
    # size=(400, 400)
)

for i in eachindex(nanogrids)
    # Dual variables (solid)
    plot!(
        p_all_nano,
        timesteps,
        λ_nano_values[i],
...
savefig(p_all_nano, "$dir/prices_and_duals_all_nanogrids")
