...
for i in nanogrids
    plot_demand = plot()
    plot!(timesteps, Array(e_dem_df[timesteps, i]), label="demand_nanogrid $i", lw=2)
    plot!(title="\n Electricity Demand in Nanogrid $i over Time\n", xlabel="\nTime", ylabel="\n e_dem (kW)")
    savefig(plot_demand, "$dir/demand_nano_$i")
end
